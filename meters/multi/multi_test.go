...
func TestMultiMeter(t *testing.T) {
	...
	if err := mp1m.Meter(testConfig.TestMeter1, u1m1, nil); err != nil {
		t.Fatal(err)
	}

	mp2 := local.NewLocalMeterProvider()
	mp2m := mp2.NewMeter(testConfig.User2)
	if err := mp2m.Meter(testConfig.TestMeter1, u2m1, nil); err != nil {
		t.Fatal(err)
	}
	...
	u1m1base, _ := checkMeter.NewMeter(testConfig.User1).GetValue(testConfig.TestMeter1, d1, d2, nil)
	u2m1base, _ := checkMeter.NewMeter(testConfig.User2).GetValue(testConfig.TestMeter1, d1, d2, nil)
	...
	if v, ok := mp1.NewMeter(testConfig.User1).GetValue(testConfig.TestMeter1, d1, d2, nil); !ok || v != u1m1expect {
	...
	if v, ok := mp2.NewMeter(testConfig.User2).GetValue(testConfig.TestMeter1, d1, d2, nil); !ok || v != u2m1expect {
